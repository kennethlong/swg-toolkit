...
>    ```
>
> 5. **Process placement (Path B — as built in Phase 0).** Under the adopted Phase-0 posture the addon runs **in the renderer process** (`sandbox:false + nodeIntegration:true + contextIsolation:false`). COOP/COEP cross-origin isolation (`crossOriginIsolated === true`) is maintained independently — it is a header policy registered before `loadURL()`, not tied to the sandbox/nodeIntegration setting. See [architecture.md](../00-overview/architecture.md#sharedarraybuffer-data-channel--confirmed-negative-cross-process-path-falsified-2026-06-22).
>
> 6. **CONFIRMED NEGATIVE — cross-process utility→renderer SAB sharing is IMPOSSIBLE in Electron 42 (falsified 2026-06-22).** The previous claim that a `SharedArrayBuffer` allocated in a utility (or main) process is shared zero-copy with the renderer is **false**. Every IPC/MessagePort mechanism throws `Error: An object could not be cloned` for a SAB. Root cause: a SAB is namable only within one agent cluster / V8 process cage; Node main/utility processes are not in the renderer's cluster (HTML spec + Electron C++ source `shell/common/v8_util.cc`). A cross-process `ArrayBuffer` passed over MessagePort is a **copy** (~450 MB/s measured, `sourceDetached=false`), not a zero-copy move. Evidence: 4-way cross-AI convergence + A/B perf measurement in [CONSULT-P0SAB-SYNTHESIS.md](../../.planning/research/CONSULT-P0SAB-SYNTHESIS.md). The Phase-0 model is native-in-renderer in-process zero-copy (Path B, ~10,600 MB/s, proven by bidirectional same-memory round-trip in Plan 00-03). The utility process is NOT on the data path; `utility-worker.ts` was removed. NOTE the architecture tension this section embodies: it allocates the SAB in **TypeScript** (`new SharedArrayBuffer`) and passes the pointer into C++, whereas Phase 0 allocates the SAB in **C++** (`Napi::SharedArrayBuffer::New`) in the renderer process — reconcile the owner/allocator when the live-sync path is built for real (Phase 3+).

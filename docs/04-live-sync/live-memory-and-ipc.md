...
>
> 6. **⚠ UNVERIFIED — utility→renderer SAB sharing is likely-negative (Phase 0 cross-write experiment, 2026-06-21).** This document assumes the `SharedArrayBuffer` data channel is genuinely **shared zero-copy** across the process that owns the addon and the renderer. When that owner is a **dedicated Electron utility process** (per CONTEXT.md D-02), a cross-AI primary-source review judged same-backing-store sharing **likely-negative**: HTML structured-clone agent-cluster rules predict `DataCloneError` (THROWS) or an independent **COPY** across the utility→renderer boundary (a utility process is a separate OS child, not in the renderer's agent cluster); Node documents SAB sharing only between threads of one process; Electron documents no utility→renderer backing-store sharing. Per AGENTS.md this caveat holds until Phase 0's same-memory **cross-write proof** passes in dev (00-03) **and** packaged (00-05). If it proves copy-only/throws, pivot per [architecture.md](../00-overview/architecture.md#sharedarraybuffer-data-channel): **main-owned SAB** / **OS shared memory** / **drop the utility from the hot path** — without re-deciding D-02/D-04 (record as a finding). NOTE the architecture tension this section embodies: it allocates the SAB in **TypeScript** (`new SharedArrayBuffer`) and passes the pointer into C++, whereas D-02 places the addon in the utility process and Phase 0 allocates the SAB in **C++** (`Napi::SharedArrayBuffer::New`) to avoid the `arrayBuffer.Data()` dangling-pointer trap in item 4 — reconcile the owner/allocator when this path is built for real.

<!--
ADOPTION & PROVENANCE (SWG-Toolkit)
-----------------------------------
Adopted into this repo 2026-08-01 from the swg-client-v2 provider session, produced at the
maintainer's (Kenny's) direction. Canonical upstream copy: swg-client-v2 `docs/research/`,
committed `fa346b421` — deltas land there first; this file is SWG-Toolkit's working copy.
Received handoff record: `.planning/handoff/2026-08-01-PROVIDER-DOC-asset-formats-and-modding-guide.md`.

GROUND-TRUTH STATUS — read this before trusting the surrounding docs/ tree:
Unlike the rest of `docs/` (Gemini-distilled; see `00-overview/source-provenance.md`), this
document is loader-by-loader VERIFIED against real swg-client-v2 engine source with file:line
citations, plus a 193,475-file `sku0_client.toc` census. Spot-checked on adoption against the
real source: `FileName.cpp` P_appearance/P_shader/P_texture prefix table and
`AppearanceTemplateList.cpp:529` (the `.apt` one-level-indirection FATAL) — both byte-accurate.
Line numbers drift over checkouts; form tags don't (the doc says so up front).

STILL FLAGGED UNVERIFIED — do NOT launder these into fact (see §12 "Known-unverified"):
`.mif` macro grammar, `.lay` internals, some `.cdf` record payloads, v6000 payload encryption,
exact server-side CmdStartScene name normalization.

§10 (SOE editor inventory + P1–P6 priority queue + gap analysis) is roadmap-shaping input, not
format reference — tracked in `.planning/backlog/guided-workflows-and-formats-roadmap-incorporation.md`.
-->

...
-----------------------------------
Adopted into this repo 2026-08-01. The AI-driven guided-wizard CONCEPT originated with the
maintainer (Kenny); this document is the design elaboration, produced in the swg-client-v2
provider session at his direction. Received handoff record:
`.planning/handoff/2026-08-01-PROVIDER-NOTE-guided-workflows-and-ai-layer-design.md`.

STATUS: design PROPOSAL, not ground truth. It aligns with — and deliberately does NOT replace —
`ai-and-mcp-integration.md` and Phase 8's AI-01 (MCP server) / AI-02 (advisory assists).

DECISIONS DEFERRED TO PLANNING (not settled here):
- The sequencing recommendation (workflow engine relative to Phase 6 Blender) and the new
  requirements WF-01 / WF-02 / AI-03 are roadmap decisions, tracked in
  `.planning/backlog/guided-workflows-and-formats-roadmap-incorporation.md`.
- The tier-C embedded-agent SDK specifics (§2.3: Tool Runner, OAuth subscription-token flow,
  model guidance) MUST be re-verified against the current Claude API reference at build time;
  the subscription-OAuth licensing question for third-party apps is explicitly OPEN.
-->

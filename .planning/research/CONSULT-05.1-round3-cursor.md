## YOUR ANGLE (Cursor — most detailed line-level code reader). Non-overlapping with other reviewers.
Do a precise line-level read of the R1-R5 fixes and the two ripple fixes the planner added:
- R2 reset: g_capKind/g_capCellName EDIT-default + 2-line reset at the F-arm call site Plan05 built — is the reset at the RIGHT site, and does EDIT-after-ADD now assemble as EDIT not ADD?
- ripple A: refresh(overrideDir, buildingTemplates?) threading — trace end to end that buildingTemplateVfsPath is non-empty at the detail card, not ''.
- ripple B: Plan12 placement path resolving g_capBuildingTemplate before assembleDecorationEdit's first line — is the resolution technique (getObjectByIdAdvertised+getTemplateFilename) actually valid there?
- R1: is ctx.studioDir?: optional in Plan06 AND made real by Plan08 — no plan left assuming it required?
Report [SEVERITY] plan — line-of-reasoning. State CONVERGED if no HIGH remains.

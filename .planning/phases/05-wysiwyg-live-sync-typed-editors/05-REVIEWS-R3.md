...
## MAINTAINER DECISIONS (2026-07-12 — resolve before the round-4 `--reviews` replan)

All five items are decided. The `--reviews` replan MUST fold every one of these in.

1. **Baseline vs. per-frame focus (HIGH) — APPLY THE PRESCRIBED FIX.** Re-capture the agent baseline +
   host COW snapshot on focus-identity change (networkId/templateName on advertised; raw `focus` pointer
   on legacy). Minimum: agent resets `s_expectedCaptured=false` when `focus` differs from the pointer it
   captured against. Restores D-03's per-object model and makes newly-selected objects editable.

2. **Gate + unify the rebaseline (HIGH) — APPLY THE PRESCRIBED FIX.** Move the REBASELINE mutation
   **inside** the `cmdSeq`-new + non-torn guard (once-per-command, atomic with apply), and rebaseline
   **both** channels to the **live** value at that gated instant so scale un-sticks like transform. Add
   the lexical-scope acceptance grep asserting the rebaseline assignment sits inside the seq-new guard.

3. **Targeting honesty (HIGH) — DECISION: OPTION (a), HUD warning + honesty.** Do all four:
   (1) render an inline `⚠ viewing <loaded>, moving <target>` HUD warning when the loaded viewport asset
   name differs from `verifiedState.templateName` (both names already available to the toolkit — no new
   resolver); (2) rewrite the 05-03 "corrects the overclaim" note so it no longer self-contradicts —
   "makes the fallback real/symmetric on both builds; does NOT close the viewport↔live-object binding gap,
   which remains open until Phase 7 `.ws`"; (3) add a 05-12 UAT step that deliberately tests the MISMATCH
   case (load asset X, select unrelated object Y in-game, confirm the ⚠ banner fires) and records the
   maintainer's acknowledgement — replace the gap-hiding "matching the live focus object" wording;
   (4) soften LIVE-03 status language wherever it is marked complete to reflect the deferred binding.

4. **Off-thread UAF (MEDIUM) — ACCEPT-WATCHED + MITIGATE.** Confirm accept-watched (defensible for a
   single-user desktop tool). Land the SEH (`__try/__except`) bound around the resolve→read→apply span so
   a UAF degrades to a skipped frame + a published fault liveness bit instead of a client crash; name the
   intermediate `hudInstance` window explicitly in the 05-12 UAT watch item.

5. **Wiring gaps (MEDIUM) + minors (LOW) — APPLY ALL.** Task the `GUARD_ADDR` (offset 392) decode in
   05-07 + a synthetic-buffer test (`0xDEADBEEF` at 392); extend `lastDiscardedChange` to per-channel
   `{transform?, scale?}`; consume bit5 `scaleGuardUnavailableOnBuild` as a fourth scale sub-state (or
   explicitly document it as agent-only until the offset handoff closes); give STOP a dedicated sticky bit
   checked independently of `cmdSeq` (or have `detachUI` spin until it observes a "stopping" liveness
   bit); fix the `network.cpp:35`→`:42` citation and the Utinni path prefixes (`swg/misc/network.cpp`,
   `swg/game/game.cpp`, `swg/ui/cui_manager.cpp`). Prefer agent-published discarded bytes for
   `lastDiscardedChange` (or soften the banner copy to not assert exact bytes).

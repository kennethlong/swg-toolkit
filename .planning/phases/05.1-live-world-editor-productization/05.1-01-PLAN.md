...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    mirrorToStockIlf=true case per kind.
  </action>
  <verify>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `packages/harness/scripts/extract-ilf-fixtures.cjs` attempts to extract ONE real `.ilf` from an
      ...
      real `.tre` archives verified on disk — then 'D:/SWGEmu Client/SWGEmu' as a fallback candidate),
  ...
  <action>
    ...
    convention, and a `beforeAll` that calls `registerFormat('ilf', {...})` with the fixtures array as
    specified in the behavior section. Write the round-trip + addNode/removeNode test cases per the behavior

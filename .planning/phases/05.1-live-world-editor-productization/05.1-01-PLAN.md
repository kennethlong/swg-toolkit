...
<task type="auto" tdd="true">
  ...
  <acceptance_criteria>
    ...
    (excluding the import line) is >= 1; full file's existing 11 other cases unchanged (OK plus the six
    SAVE_* codes, NODE_NOT_FOUND, BUILDING_ID_MISMATCH, ABORTED, NOT_A_WS_NODE — ROUND-3-REVIEW R13/Fable L3
    corrects the prior "10" miscount) (diff shows one inserted line, zero deletions in the switch body).
  </acceptance_criteria>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - **(ROUND-3-REVIEW R7, MEDIUM, Codex+Sonnet — disclosure, not a behavior change):** because addNode is
      strictly append-only by design, ANY caller that re-adds a previously-removed node via addNode — including
      Plan 13's Undo, which reuses this SAME code path per D-01's "one code path" mandate — places the
      re-added row at the END of its cell, not at its original position. This reorders the cell's later rows'
      effective indices. This is an ACCEPTED, disclosed consequence of append-only addNode (see Plan 13's Undo
      behavior spec and Plan 15's sign-off ledger for the explicit scoping), not a defect in this task's
      implementation — addNode itself is not revised to support positional insert.

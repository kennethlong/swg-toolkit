...
**R9 REVISION (2026-08-01, REVIEWS.md round 9 — BB4/BB10; citation convention: in-plan revision labels and
REVIEWS-round numbers are different sequences — review items are cited as 'R9 review, BBn'):** Task 2's
dispatch gains the epoch-0 tracker-reset rule — BB4's one REAL residual after the review's persistence
premise was adjudicated against ground truth and falsified (`openChannel` unconditionally zero-fills the
whole mapping, `channel_binding.cpp:153-154`, so no REGION epoch survives a toolkit restart; only the
AGENT-LOCAL tracker does) — and an explicit always-acks statement on its publish tail (BB10). The canonical
ack contract both cite lives in Plan 08's HOST_CMD ACK PROTOCOL section.
...
<task type="auto">
  ...
  <behavior>
    - Once per frame (called from renderFrame(), alongside applyPendingRebind()), read a fresh HOST_CMD epoch
      via channelReadHostCommand; if epoch is 0, RESET the last-applied-epoch tracker to 0 and no-op (R9
      review, BB4 — a zeroed epoch slot means a fresh `openChannel` just re-initialized the region:
      `channel_binding.cpp:153-154`'s unconditional `std::memset(view, 0, CHANNEL_BYTE_SIZE)` runs on EVERY
      open, including a re-open while this agent is still running, so no region epoch survives a toolkit
      restart — resetting here guarantees a restarted toolkit's epochs 1..N can never silently collide with
      a stale tracker from a previous toolkit session, per Plan 08's HOST_CMD ACK PROTOCOL 'Epoch lifetime'
      clause); if epoch equals the last-applied epoch, no-op (same once-per-epoch contract as
      applyPendingRebind).
    ...
    - After handling, publish the result via channelWriteHostCommandResult(code, epoch) and update the
      last-applied-epoch tracker. **(R9 review, BB10)** This tail IS the 'agent always-acks every consumed
      epoch' invariant of Plan 08's HOST_CMD ACK PROTOCOL: every branch of this dispatch — including every
      refusal/no-op branch a later plan adds (Plan 12's exclusivity guards) — must route through this
      publish; a literal `return;` that skips it strands the renderer's pending-ack slot until its timeout
      fires.
  </behavior>
  <action>
  ...
  <acceptance_criteria>
    ...
    missing-default crash risk); by inspection, the epoch==0 branch resets the last-applied tracker to 0
    before no-opping (R9 review, BB4), and NO branch of the dispatch returns without reaching
    channelWriteHostCommandResult (R9 review, BB10).
  </acceptance_criteria>
...
| T-05.1-09b | Denial of Service | unrecognized HOST_CMD_ACTION value | mitigate | Explicit fail-closed default branch (behavior spec) — never crashes, never spins, always publishes a result so the host-side epoch tracking never stalls waiting for an ack that will never come — the 'agent always-acks' clause of Plan 08's HOST_CMD ACK PROTOCOL (R9 review, BB10). |

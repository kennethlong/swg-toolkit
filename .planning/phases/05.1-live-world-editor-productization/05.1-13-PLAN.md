...
must_haves:
  truths:
    ...
    - "The Remove click handler resolves overrideDir/readVfs/worldEditorBuildingTemplates FRESH at click time and refreshes the tree via Plan 10's shared refreshTree() helper, never a cached mount-time value (ROUND-3-REVIEW R2)"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - **(ROUND 3, R4/R7)** WorldPanel.tsx: each decoration row gains a "Remove" action (icon/button, per-row,
      no confirm dialog — D-03). **(ROUND-3-REVIEW R2, HIGH, Cursor)** Its click handler resolves a FRESH
      `overrideDir`/`readVfs`/`meta` pair by calling Plan 10's shared `refreshTree()`-adjacent resolution
      (`readWorkspaceJson(studioDir)` read FRESH at click time, then `resolveScanRoot(...)`/`makeReadVfs(overrideDir)`
      — the SAME pattern Plan 10's mirror-toggle onChange uses, reuse it, do not invent a second resolution
      path, and NEVER reuse a `meta`/`overrideDir` value cached from mount or from a prior click), and reads
  ...
  <acceptance_criteria>
    ...
    test's fake-timer idiom); the recorded history entry for a removal is always `outcome: 'warn'` with a
    ...
    the undo toast (ROUND 3/R3). **(ROUND-3-REVIEW R2)** clicking Remove resolves `overrideDir`/`readVfs` via a
    FRESH `readWorkspaceJson` read at click time (asserted via a spy showing `readWorkspaceJson` is called
    AGAIN on a second Remove click, not reused from a cached value) and, on success, calls the SAME shared
    `refreshTree()` helper Plan 10's mount effect and mirror toggle use (asserted via a spy on `refreshTree`,
    not a direct `refresh()` call with a stale argument pair).
  </acceptance_criteria>

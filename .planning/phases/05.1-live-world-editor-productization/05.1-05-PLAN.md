...
<task type="auto">
  ...
  <read_first>
    ...
    function is the one-line D-10 stash described below)
    packages/live-inject/agent/channel.h (Plan 03 Task 3 — `DECO_REBIND_FLAG_MIRROR_OFF`)
  ...
  <action>
    ...
    change); branch the saved-state text on it as specified.
  </action>
  <verify>
    <automated>cmake --build packages/live-inject/agent/build-agent --config Release</automated>
  </verify>

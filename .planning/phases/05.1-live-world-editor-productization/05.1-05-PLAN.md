...
**ROUND 4 REVISION (2026-08-01, REVIEWS.md W5 — LOW, Fable; citation-only, no behavior change):** Task 2's
action text cited `persistDecorationEdit`'s `channelWriteCapture(&cap, ++g_captureEpoch);` call as "line 444";
verified this session against the real file, that call sits at `overlay.cpp:443` — line 444 is the function's
`return nullptr;`. Fixed below. The `g_captureEpoch` shared-counter claim itself was already correct; only the
line number was off by one.
...
passes to `channelWriteCapture`, at `overlay.cpp:443` — verified this session, ROUND 4/W5).
...
<task type="auto">
  ...
  <read_first>
    ...
    `DecorationCapture cap = {}; ...; channelWriteCapture(&cap, ++g_captureEpoch);` call shape (at
    `overlay.cpp:443` — verified this session, ROUND 4/W5) this task's arm-failure publish must mirror, using
    the SAME shared `g_captureEpoch` counter)
  ...
  <behavior>
    ...
    - hover: g_lastRayObj is non-null and nothing is armed. Strip shows the object label + "press F to arm".
      ...
      **published off the agent process** via `channelWriteCapture` with `cap.kind = DECO_CAPTURE_KIND_ARM_FAILED`
      ...
      `++g_captureEpoch` (the SAME shared counter `persistDecorationEdit` uses at `overlay.cpp:443` — a fresh
      epoch either way is "a new capture event," regardless of which kind it carries). This is the C8 fix: an
      arm failure is no longer a dead-end local stash — it reaches the renderer's existing CAPTURE poll
      exactly like a real persist capture does, where Plan 06's orchestrator routes it to
      `worldEditorStore.recordArmFailure`.
  ...
  <action>
    ...
    persistDecorationEdit's own call shape at `overlay.cpp:443` — ROUND 4/W5 citation fix; was previously
    misquoted as line 444 — reusing the same shared epoch counter).
  </action>
  <verify>
    <automated>cmake --build packages/live-inject/agent/build-agent --config Release</automated>
  </verify>

...
must_haves:
  truths:
    ...
    - "A partial/failed reconcileMirrorMode result is surfaced DURABLY — a worldEditorStore.history entry, visible in the Activity accordion for the rest of the session, not merely a transient log() line — and the toggle's visual state reverts to match what Plan 06 actually persisted (ROUND-3-REVIEW R6, Plan-10 half)"
...
field (`liveStore.ts:237`, set at `:394`) — read it via its OWN selector, matching the existing accessor
`useChannelReader.ts:173` already uses. Using `liveStatus.clientLabel` directly is a `tsc` error, not a
runtime nuance — do not write that access pattern anywhere in this plan):
```typescript
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - **(ROUND-3-REVIEW R2/R8-M3)** On mount, and whenever `liveStatus.kind` changes (attach/detach), calls the
      shared `refreshTree()` helper (defined in this task's Action section below) — which reads
      `readWorkspaceJson(studioDir)` FRESH each time, resolves `overrideDir` via `resolveScanRoot`, and either
      calls `scanWorldEditorState`-backed `refresh(overrideDir, meta.worldEditorBuildingTemplates)` (Plan 04's
      widened signature — ROUND 3, R3 seam) when `overrideDir` resolves to a real path, or renders a words-only
      "no live session / project not bound to a client" disabled state when `resolveScanRoot` returns `null`
      (ROUND-3-REVIEW R8/M3 — a normal, non-error D-07b state, never an unhandled crash). `refresh`'s SECOND
      argument is ALWAYS the FRESHLY-read `meta.worldEditorBuildingTemplates` — never a value captured once at
      mount and reused later (ROUND-3-REVIEW R2, HIGH, Cursor: a stale mount-time `meta` would starve `refresh()`
      of any `worldEditorBuildingTemplates` entries a LATER live persist writes, leaving `buildingTemplateVfsPath`
      empty on the tree even after a real capture populated the durable map — defeating R3 end-to-end). The
      live-session strip's "refresh" control (019-A's sketch element of that name) calls `refreshTree()`
      directly on click, giving the user a manual re-scan affordance alongside the automatic mount/attach-change
      triggers.
    ...
    - **(ROUND 3, R4/R6)** Mirror-mode toggle row: a switch reflecting the current project's `mirrorToStockIlf`
      (default true when absent); toggling it resolves a FRESH `overrideDir`/`readVfs` pair via the SAME
      `readWorkspaceJson`+`resolveScanRoot(liveStatus.kind === 'attached' ? clientLabel ?? null : null, {
      cfgPath, clientPath })` logic `refreshTree()` performs (ROUND-3-REVIEW R2 — do not read a cached `meta`
      captured at mount) and builds `readVfs` via `makeReadVfs(overrideDir)`, then calls
      `reconcileMirrorMode(studioDir, overrideDir, readVfs, nextValue)` — this is the ONLY place
      `mirrorToStockIlf` gets persisted (`reconcileMirrorMode` itself calls `updateWorkspaceMeta` internally,
      per Plan 06); this panel's onChange handler does NOT ALSO call `updateWorkspaceMeta` directly (R6 —
      single owner, closing the two-writers-two-moments risk D-09 exists to prevent). The handler consumes
      `reconcileMirrorMode`'s RETURNED `{ failures }` (R8) and, when non-empty: **(ROUND-3-REVIEW R6, Plan-10
      half — durable, not just transient, failure surfacing)** pushes a `recordPersistResult`-style entry into
      `worldEditorStore.history` (`outcome: 'error'`, message naming which building ids failed and why, built
      from the `failures` array — visible in the Activity accordion for the rest of the session, not just a
      console/log-panel line that scrolls away) AND visually reverts the switch to its PREVIOUS `mirrorToStockIlf`
      value (since Plan 06's R6 fix means the flag was NOT persisted on a partial failure — the toggle's visual
      state must not silently disagree with what actually landed on disk/settings). A `log('warn', 'log', ...)`
      line is ALSO still emitted (console-visible trace), but the history entry is the PRIMARY, durable surface.
      On completion (clean OR partial), calls `refreshTree()` (ROUND-3-REVIEW R2) so the rendered tree reflects
      whatever the reconcile actually changed on disk; a hint line reads "per-template: all buildings with this
      layout show the edit" when ON; a `PER-TEMPLATE` badge is shown (matching 019-A).
  ...
  <action>
    ...
    `tsc` error).
    **(ROUND-3-REVIEW R2, HIGH, Cursor)** Define ONE shared `refreshTree()` function in this component's body
    (not inlined separately at each call site) that: (1) reads `readWorkspaceJson(studioDir)` FRESH — never a
    value captured in an outer closure/ref from a previous render or a previous call — into a local `meta`; (2)
    resolves `overrideDir` via `resolveScanRoot(liveStatus.kind === 'attached' ? clientLabel ?? null : null, {
    cfgPath: meta.cfgPath, clientPath: meta.clientPath })` (ROUND 3, MED-11); (3) **(ROUND-3-REVIEW R8/M3,
    Opus)** if `overrideDir === null` (unbound project or fully offline with no cfgPath — a normal D-07b state,
    per `resolveScanRoot`'s ROUND-3-REVIEW R9 null-return contract, Plan 04), do NOT call `refresh()` at all —
    ...
    precedent for an unresolvable scan root; (4) otherwise calls `refresh(overrideDir,
    meta.worldEditorBuildingTemplates)` (Plan 04's widened signature — ROUND 3 seam). `refreshTree()` is called
    from: a mount `useEffect` (re-running when `liveStatus.kind` changes, so attach/detach re-resolves the scan
    root); the live-session strip's "refresh" control (019-A's sketch element of that name, previously
    unwired — clicking it calls `refreshTree()` directly); and is the SAME function Plans 13/14 (later waves,
    same file) MUST call for their own post-remove / history-triggered refreshes — Plans 13/14 do NOT read
    `readWorkspaceJson` or resolve `overrideDir` a second, independent way; they call this component's
    `refreshTree()`. NO call site anywhere in this file (mount, live-strip refresh button, mirror toggle,
    Plan 13's post-remove, Plan 14's history effect) may cache `meta`/`overrideDir` across renders/calls for
    reuse in a LATER refresh — each `refreshTree()` invocation re-reads `readWorkspaceJson` fresh (ROUND-3-REVIEW
    R2's core fix: a stale, mount-time-only `meta` silently starves `refresh()` of any `worldEditorBuildingTemplates`
    entries written by a persist that happened AFTER mount).
    Implement the mirror toggle's onChange to resolve the SAME `overrideDir`/`readVfs` via a FRESH
    `readWorkspaceJson`+`resolveScanRoot` read (via `makeReadVfs(overrideDir)`, Plan 06 ROUND 3's now-exported
    factory), then call ONLY `reconcileMirrorMode(studioDir, overrideDir, readVfs, nextValue)` (ROUND 3, R4/R6 —
    do NOT also call `updateWorkspaceMeta` from this panel; `reconcileMirrorMode` is the sole persistence owner),
    consuming its returned `{ failures }` (ROUND 3, R8) to surface any per-building failures via the existing
    `log()` service — wrapped in a try/catch as defense-in-depth (do not let a reconcile failure crash the
    panel), but the PRIMARY failure signal is the return value, not the catch; then calls `refreshTree()` so the
    panel's tree/mirror-derived display reflects the just-reconciled disk state immediately. Render the disabled
    ...
    `worldEditorStore.history` with `outcome: 'error'` naming the same failing building(s), asserted on the
    ...
    R6); clicking a detail-card stub button
  ...
  <acceptance_criteria>
    ...
    consumes its `{ failures }` return value. R5/ROUND-3-REVIEW R4 assertion: the detail card renders D-13's
  ...
  (R8); the detail card renders D-13's FULL 12-element before/after-transform + cell/row readout, proven
  visibly-different on a rotate-only edit (ROUND-3-REVIEW R4, superseding the round-2 translation-only framing)
  and its inert stub buttons show honest, non-misleading copy (R11a); row selection uses one shared id contract
  (MED-7). Every `refresh()` call site (mount, manual refresh, mirror toggle) re-reads `worldEditorBuildingTemplates`
  fresh via a shared `refreshTree()` helper, never a stale mount-time value (ROUND-3-REVIEW R2); `clientLabel` is
...
an unresolvable scan root degrades to a words-only disabled state (R8/M3).
</success_criteria>

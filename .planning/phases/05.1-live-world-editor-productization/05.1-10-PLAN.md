...
<task type="auto" tdd="true">
  ...
  <behavior>
    - **(ROUND 4, W3 — MEDIUM, Cursor)** ONE shared `resolveOverridePair(): { overrideDir: string; readVfs:
      (vfsPath: string) => Buffer; meta: WorkspaceBindingMeta } | null` helper is defined in this component's
      body (co-located with `refreshTree()`), carrying the SAME null-guard as `refreshTree()`'s own resolution:
      it reads `readWorkspaceJson(studioDir)` FRESH into `meta`, resolves `overrideDir` via
      `resolveScanRoot(liveStatus.kind === 'attached' ? clientLabel ?? null : null, { cfgPath: meta.cfgPath,
      clientPath: meta.clientPath })`, and returns `null` immediately (no `readVfs` constructed, no caller
      proceeds) when `overrideDir === null` — otherwise returns `{ overrideDir, readVfs:
      makeReadVfs(overrideDir), meta }`. EVERY call site in this file that needs a real `overrideDir`/`readVfs`
      pair — `refreshTree()`, the mirror toggle's onChange, and (Plan 13, later wave, same file) the Remove
      action — calls THIS helper, never re-resolving `overrideDir`/`readVfs` inline a second way. This closes
      W3: the previous draft guarded `refreshTree()`'s resolution against a `null` `overrideDir` but left the
      mirror toggle's onChange resolving inline with no guard, so a `null` `overrideDir` (unbound/offline
      project) would have fed `string | null` into `makeReadVfs`/`reconcileMirrorMode`'s `string`-typed
      parameters — a real `tsc --noEmit` failure, not just a runtime nuance.
    ...
    - **(ROUND 3, R4/R6; ROUND 4, W1/W3)** Mirror-mode toggle row: a switch reflecting the current project's
      ...
      surfacing)** pushes a `recordPersistResult`-style entry into `worldEditorStore.history` (`outcome:
      'error'`, message reading, e.g., "mirror mode change blocked — no buildings' mirrors were changed:
      {building ids and reasons from `failures`}" — per Plan 06's ROUND 4/W1 transactional redesign, a
      non-empty `failures` NOW ALWAYS means every building's mirror is provably unchanged this call (either
      blocked before any write, or rolled back after a mid-pass throw), so the message must never imply a
      partial per-building outcome the underlying function no longer produces) AND visually reverts the switch
  ...
  <action>
    ...
    `tsc` error).
    **(ROUND 4, W3, MEDIUM, Cursor)** Define ONE shared `resolveOverridePair()` function in this component's
    body (per the behavior spec above) that reads `readWorkspaceJson(studioDir)` FRESH — never a value captured
    in an outer closure/ref from a previous render or a previous call — resolves `overrideDir` via
    `resolveScanRoot(liveStatus.kind === 'attached' ? clientLabel ?? null : null, { cfgPath: meta.cfgPath,
    clientPath: meta.clientPath })` (ROUND 3, MED-11), and returns `null` immediately (constructing NO
    `readVfs`) when `overrideDir === null` — otherwise returns `{ overrideDir, readVfs: makeReadVfs(overrideDir),
    meta }`. **(ROUND-3-REVIEW R2, HIGH, Cursor)** Define `refreshTree()` calling `resolveOverridePair()`: if it
    returns `null` (unbound project or fully offline with no cfgPath — a normal D-07b state, per
    `resolveScanRoot`'s ROUND-3-REVIEW R9 null-return contract, Plan 04), do NOT call `refresh()` at all —
    instead leave the tree at its last-known value and render the SAME words-only "no live session / project
    not bound to a client" disabled state the Scene accordion (Plan 11) already establishes as this panel's
    precedent for an unresolvable scan root; otherwise calls `refresh(pair.overrideDir,
    pair.meta.worldEditorBuildingTemplates)` (Plan 04's widened signature — ROUND 3 seam). `refreshTree()` is
    called from: a mount `useEffect` (re-running when `liveStatus.kind` changes, so attach/detach re-resolves
    the scan root); the live-session strip's "refresh" control (019-A's sketch element of that name,
    previously unwired — clicking it calls `refreshTree()` directly); and is the SAME function Plans 13/14
    (later waves, same file) MUST call for their own post-remove / history-triggered refreshes — Plans 13/14 do
    NOT read `readWorkspaceJson` or resolve `overrideDir` a second, independent way; they call this component's
    `refreshTree()` (and, for Plan 13's Remove specifically, this component's `resolveOverridePair()` too — see
    Plan 13's own ROUND 4 revision). NO call site anywhere in this file (mount, live-strip refresh button,
    mirror toggle, Plan 13's post-remove, Plan 14's history effect) may cache `meta`/`overrideDir` across
    ...
    history-entry message must read as an ALL-OR-NOTHING outcome (ROUND 4/W1 — "no buildings' mirrors were
    changed", never per-building success/failure mixed wording, since Plan 06's transactional redesign makes
    that mix impossible); then calls `refreshTree()` so the panel's tree/mirror-derived display reflects the
  ...
  buttons show honest, non-misleading copy (R11a); row selection uses one shared id contract (MED-7). Every
...
all-or-nothing (never partial) outcome, matching Plan 06's transactional redesign (W1).
</success_criteria>

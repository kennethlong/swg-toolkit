...
must_haves:
  truths:
    ...
    - "The placement-ack protocol (epoch lifetime, pending-request slot, timeout/fallback, agent-always-acks, project-switch abort, durable failure surface) has exactly ONE canonical spec — this plan's HOST_CMD ACK PROTOCOL section — and every other plan (04/07/09/12/14/15) restates it only by citation, never by paraphrase (R9 review, BB1/BB4/BB5/BB6/BB7/BB10/BB12 — the round-9 consensus-theme-1 design pass)"
  artifacts:
...
spot-fixed plan-by-plan for five consecutive rounds. This plan's own two tasks are mechanically UNCHANGED
except one grounding note in Task 2 (why the `lastHostCmdResultEpoch = 0` baseline is correct — BB4's
...
## HOST_CMD ACK PROTOCOL — CANONICAL SPEC (R9 review: BB1, BB4, BB5, BB6, BB7, BB10, BB12)

This section is the ONE authoritative statement of the placement-ack protocol. Plans 04/07/09/12/14/15
restate pieces of it BY CITATION ("per Plan 08's HOST_CMD ACK PROTOCOL"), never by paraphrase; any conflict
between another plan's wording and this section resolves in this section's favor. Implementation ownership:
the send/observe primitives are THIS plan's Tasks 1/2 (unchanged); the renderer-side consumer state machine
is Plan 14 Task 3; the agent-side always-ack and epoch-0 reset rules are Plans 09/12.

**State machine (renderer-side; one placement request pending at a time):**

- `idle -> pending(E)`: WorldPanel calls `sendStartPlacement(...)`, which returns epoch `E`;
  `pendingPlacementRef.current = { epoch: E, buildingId, buildingLabel, templatePath, cellName }` and a
  `PLACEMENT_ACK_TIMEOUT_MS = 10_000` timeout starts. NO toast is shown at send time (BB1 — Plan 14 Task 2
  defines the success-toast COPY; Plan 14 Task 3 alone decides WHEN any toast shows).
- `pending(E) — local refuse (BB7)`: a second "Place in game ▸" attempt while `pendingPlacementRef.current`
  is non-null does NOT call `sendStartPlacement` at all — it shows the words-only toast "a placement request
  is still waiting for the game's response — try again in a moment" and leaves the pending slot untouched.
  This is what makes a SINGLE-slot ref sufficient: the slot can never be overwritten while occupied, so the
  round-9 double-click inversion (ack {5,1} ignored, {6,0} matched -> "refused" shown while placement IS
  armed) is structurally unreachable. The slot cannot wedge, because every transition below clears it —
  timeout included.
- `pending(E) -> resolved-success`: `worldEditorStore.lastHostCommandResult` matches `{ epoch: E, code: 1 }`
  -> clear slot + timeout -> show the success toast (Plan 14 Task 2's wrong-room-warning copy).
- `pending(E) -> resolved-refused (BB12)`: a matching `{ epoch: E, code != 1 }` -> clear slot + timeout ->
  show the words-only refusal toast AND record a DURABLE history entry (see "Durable failure surface"
  below) — a transient toast alone is not an acceptable failure surface, per D-12's badge-and-detail-waiting
  contract and the C8 `recordArmFailure` precedent.
- `pending(E) -> timed-out (BB6)`: no matching ack within `PLACEMENT_ACK_TIMEOUT_MS` -> clear slot -> show
  the words-only timeout toast AND record a durable history entry. The timeout is MANDATORY because delivery
  is genuinely lossy: the result slot is a single overwritten word-pair (two host commands inside one ~1 Hz
  background-throttle window lose one result — `useChannelReader.ts:114-118`'s documented Chromium timer
  throttling while the game holds the foreground), and the agent is silent during load/login/zoning. The
  deferred toast will also most likely render while the user is alt-tabbed into the fullscreen game — which
  is exactly why the durable record, not the toast, is the guaranteed surface.
- `pending(E) -> aborted-detach (BB6)`: `liveStore.status` leaves `'attached'` while pending (client exit
  stops the poll loop entirely — `useChannelReader.ts:242-250`) -> clear slot + timeout -> record a durable
  history entry ("client detached before the placement was acknowledged"). Without this, a pending request
  would wait forever on a poll loop that no longer runs.
- `pending(E) -> aborted-switch (BB5)`: `studioDir` changes while pending -> clear slot + timeout, show NO
  toast and record NO history entry — the stores were just reset for the NEW project (Plan 04's
  PROJECT-SWITCH RESET ORDERING CONTRACT), and recording Project A's placement outcome into Project B's
  freshly-reset history would be exactly the cross-project misattribution BB5 exists to close.

**Epoch lifetime (BB4 — adjudicated against ground truth this round):** one toolkit session, and the
reader's `lastHostCmdResultEpoch = 0` baseline is CORRECT as-is. The round-9 review's BB4 scenario ("the
agent's `hostCmdResultEpoch` persists in the still-running client at e.g. 47; a restarted toolkit's acks are
silently dropped until its counter passes 47") is FALSIFIED by real source: `openChannel`
(`channel_binding.cpp:113-170`) calls `CreateFileMappingA` and then UNCONDITIONALLY zero-fills the entire
mapping (`std::memset(view, 0, CHANNEL_BYTE_SIZE)`, `:153-154`) — including when the named mapping already
exists because a still-running agent holds it — so no REGION epoch (command or result) ever survives a
toolkit restart. The one REAL cross-restart residual is the AGENT-LOCAL last-applied-epoch tracker (agent
process memory, Plan 09's dispatch), which persists (e.g. at 47) while a restarted renderer counts up from 1
again: the original "equal to last-applied -> no-op" rule would silently swallow — with no ack — exactly the
ONE command whose epoch collides with the stale tracker. Closed agent-side (Plan 09's R9 revision): when
`channelReadHostCommand` observes HOST_CMD epoch 0 (the zero-filled slot every fresh `openChannel`
produces), the agent resets its last-applied tracker to 0, so a new session's epochs 1..N never collide.
Disclosed micro-residual: a send issued in the same instant as `openChannel`, before the agent's next
~16 ms frame observes the zeroed slot, could still collide — unreachable in practice across the
seconds-long attach->UI->click path, and even then converted by the timed-out transition into a visible,
durable failure, never silence.
...
refusal into a 10-second silence instead of an instant honest answer).
...
any pending placement per `aborted-switch` above (Plan 14 Task 3 — the pending slot and its timer are
component state, unreachable by any store reset).

**Durable failure surface (BB12):** the refused / timed-out / detached transitions each call
`useWorldEditorStore.getState().recordPersistResult` with `outcome: 'error'` (setting `hasFailureBadge` —
D-12: the badge waits for the user, never steals focus), `buildingLabel` = the pending request's building
resolved from the tree (`tree.find(b => b.buildingId === pending.buildingId)?.displayLabel ?? <the raw
id>`), `decorationLabel` = the pending `templatePath`'s basename, `timestampISO` = now, and a words-only
`message` (never a raw code, SC1): refused -> "placement request was refused — an edit or placement may
already be active in-game; cancel it, then try again"; timed-out -> "placement request got no response from
the game (it may be loading, zoning, or busy) — if the placement ghost is active in-game, placing and
persisting will still work; otherwise try again"; detached -> "client detached before the placement was
acknowledged". The success transition records NO history entry (the eventual persist RESULT itself does,
via this plan's Task 3 wiring).
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    that a repeat poll of the SAME epoch does not call it again (BLOCKER fix regression test).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    fix — the producer half of AA4's ack-correlation contract).
  </acceptance_criteria>
...
gap.
R9: this plan additionally hosts the ONE canonical HOST_CMD ACK PROTOCOL spec (state machine, epoch
lifetime, timeout/fallback, always-acks, switch-abort, durable failure surface) that Plans 04/09/12/14/15
cite instead of paraphrasing (BB1/BB4-BB7/BB10/BB12) — including the ground-truth adjudication that
FALSIFIED BB4's region-epoch-persistence premise (`channel_binding.cpp:153-154`'s unconditional zero-fill)
while closing its real agent-local-tracker residual in Plan 09.
</success_criteria>

...
## HOST_CMD ACK PROTOCOL — CANONICAL SPEC (R9 review: BB1, BB4, BB5, BB6, BB7, BB10, BB12; R10 review: CC1, CC2, CC3, CC5, CC7, CC13, CC14, CC17 — respecified as a TOTAL state × event table, not a trigger list)
...
**Timeout constant (R10 review, CC3 — latency-aware):** `PLACEMENT_ACK_TIMEOUT_MS = 11_000`, derived as a
10_000 ms ack-PUBLISH budget (the agent's frame loop answers in ~16 ms when responsive; the budget exists
for ordinary in-game latency) PLUS 1_000 ms worst-case READ lag: the deadline bounds when the renderer
READS the ack, not when the agent publishes it — under the documented ~1 Hz background throttle
(`useChannelReader.ts:114-118`: Chromium throttles the ~33 ms poll to ~1/s while the game holds the
foreground), an ack published just inside the publish budget is read up to one full throttled poll interval
later, so the deadline adds at least that interval on top. The timeout `setTimeout` itself is throttled the
same way and can therefore only fire LATE, never early — throttling widens the grace, never narrows it. The
honest residual (the agent is SILENT during load/login/zoning, which routinely exceeds ANY fixed deadline)
is handled by the hedged Canonical strings below and disclosed as Plan 15's gap ledger item (n).
...
| 10 | pending(E) | detach (`liveStore.status` leaves `'attached'` — client exit stops the poll loop entirely, `useChannelReader.ts:242-250`) | idle | Clear slot + timer; record the durable detached entry (Canonical string 3). Fires only for acks that truly never arrived: the poll loop reads the channel BEFORE the throttled liveness check on the same tick (Task 2's read-before-liveness ordering, R10 review CC5), so an ack the agent published before exiting resolves as row 5/6, never as a false 'detached' record. Without this row, a pending request would wait forever on a poll loop that no longer runs. |
...
correct). Enforcement is LOCAL, never hook-declaration-order dependent: the `studioDir`-carrying slot
(row 7) makes ack-vs-switch self-resolving; every exit clears the timer, making timeout-fire lose to any
completed exit; and the timeout callback's re-check (row 9) makes it lose even to a same-tick exit.
...
list (R10 review, CC1/CC2/CC3/CC7/CC13/CC14/CC17) — which Plans 04/07/09/12/14/15 restate under the CC8

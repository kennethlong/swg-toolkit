...
| 5 | pending(E) | ack(E, 1) with `slot.studioDir === useWorkspaceStore.getState().studioDir` | idle | Clear slot + timer; show the success toast (Plan 14 Task 2's wrong-room-warning copy). NO history entry — the eventual persist RESULT records its own, via this plan's Task 3 wiring. |
| 6 | pending(E) | ack(E, code != 1) with matching `studioDir` | idle | Clear slot + timer; show the refusal toast AND record the durable refusal entry — both use Canonical string 1 (see Durable failure surface). |
| 7 | pending(E) | ack(E, any code) with `slot.studioDir !== useWorkspaceStore.getState().studioDir` | idle | ABORT, not resolution (CC2's local enforcement): clear slot + timer, NO toast, NO record — the stores were already synchronously reset for the new project; resolving would write Project A's outcome into Project B. This row is what makes the precedence rule independent of effect ordering: even when an overdue ~1 Hz poll tick lands A's refusal in the just-reset store BEFORE the passive-effect flush, the ack effect itself self-converts to the abort. |
| 8 | pending(E) | ack(epoch != E) | pending(E) | Self-loop: ignore (a stale/unrelated result); slot, timer, and pending UI signal untouched. |
| 9 | pending(E) | timeout-fire (the callback re-checks: slot non-null AND `slot.epoch === E` AND `slot.studioDir` matches current — else no-op) | idle | Clear slot; show the timeout toast AND record the durable timed-out entry (Canonical string 2). The timeout is MANDATORY because delivery is genuinely lossy: the result slot is a single overwritten word-pair (two host commands inside one ~1 Hz background-throttle window lose one result — `useChannelReader.ts:114-118`), and the agent is silent during load/login/zoning. The deferred toast will most likely render while the user is alt-tabbed into the fullscreen game — which is exactly why the durable record, not the toast, is the guaranteed surface WITHIN the project session (durability scope disclosed under Durable failure surface, CC4). |
| 10 | pending(E) | detach (`liveStore.status` leaves `'attached'` — client exit stops the poll loop entirely, `useChannelReader.ts:242-250`) | idle | FIRST re-check `slot.studioDir === useWorkspaceStore.getState().studioDir` (the SAME local guard as rows 7/9 — R11 review, EE2: detach is store-writing too, so it enforces the switch-abort locally rather than relying on effect declaration order): on a mismatch, silent abort — clear slot + timer, NO toast, NO record, the same shape as row 7's aborted-switch conversion (a 'detached' record would land in the NEW project's freshly-reset history). On a match: clear slot + timer; record the durable detached entry (Canonical string 3). Fires only for acks that truly never arrived: the poll loop reads the channel BEFORE the throttled liveness check on the same tick (Task 2's read-before-liveness ordering, R10 review CC5), so an ack the agent published before exiting resolves as row 5/6, never as a false 'detached' record. Without this row, a pending request would wait forever on a poll loop that no longer runs. |
| 11 | pending(E) | project-switch (`studioDir` changes) | idle | Clear slot + timer; NO toast, NO record (aborted-switch, BB5) — the stores were just reset for the NEW project (Plan 04's PROJECT-SWITCH RESET ORDERING CONTRACT); recording Project A's outcome into Project B's freshly-reset history is exactly the cross-project misattribution this row exists to close. |
| 12 | pending(E) | unmount | (component destroyed) | `aborted-unmount` (R10 review, CC1): Plan 14 Task 3's mount-lifetime `useEffect` CLEANUP clears the timer and the slot dies with the component — NO record, NO toast (a timeout callback surviving the component could fire after a later project switch and write `outcome: 'error'` + `hasFailureBadge` into the NEW project's freshly-reset history through the module-scope store — CC1's exact resurrection sequence, structurally closed here). A later ack for E lands in row 2 and is dropped. |
...
timed-out / detached transitions (rows 6/9/10) each call `useWorldEditorStore.getState().recordPersistResult`

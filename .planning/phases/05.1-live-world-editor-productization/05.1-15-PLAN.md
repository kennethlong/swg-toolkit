...
round-3's own fixes interacting with each other rather than by any fabrication (Codex/Opus/Fable all confirmed
zero fabrications in the round-3→round-4 replan). Both are now CLOSED by targeted fixes in their source plans,
    ...
    **Gap ledger — explicit recording, not a silent pass:**
    12. Record each of the following as "observed, accepted" (or "observed, NOT accepted — escalate") in your
        ...
        **(ROUND-3-REVIEW R10b)** REMOVE's actual eligible scope was narrower than D-02's literal "any decoration
        ...
        own (b) narrowing above (expected). If ANY of these behaved DIFFERENTLY than described (better OR
        worse), record that explicitly — it changes the phase's actual state.
...
cites exact ground-truth references; the blocking checkpoint's 11 verification steps all pass and step 12's

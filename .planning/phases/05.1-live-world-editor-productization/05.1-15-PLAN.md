...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <resume-signal>Type "approved" once all 8 checks hold (recording any SC2 sketch-element gaps found even if
  minor), or describe which check failed / which sketch element is missing.</resume-signal>
</task>
...
<verification>
The change-request handoff file exists and cites exact ground-truth references; the blocking checkpoint's 8
verification steps all pass (with any SC2 sketch-element gaps explicitly recorded, not silently ignored).
</verification>

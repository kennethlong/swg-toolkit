...
<task type="auto" tdd="true">
  ...
  <done>Agent can read a fresh HOST_CMD epoch and publish a result, using the exact proven seqlock idioms.
  NOTE (R9 review, BB4): `hostCmdResultEpoch` lives in the SHARED REGION, which the host's `openChannel`
  unconditionally zero-fills on every open (`channel_binding.cpp:153-154`) — no C++ change is needed HERE
  for toolkit-restart epoch hygiene; the agent-side last-applied-tracker zero-reset rule lands in Plan 09's
  dispatch, per Plan 08's HOST_CMD ACK PROTOCOL (the canonical ack spec these primitives feed).</done>
</task>

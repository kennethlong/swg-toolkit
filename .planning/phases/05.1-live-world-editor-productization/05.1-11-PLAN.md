...
type: execute
wave: 3
depends_on: ["05.1-10", "05.1-08", "05.1-02", "05.1-18"]
depends_on_note: "05.1-18 added 2026-08-02 — D-07 teleport bookmarks are built on a teleport carrying two known defects; 18 fixes the stale-player-pointer half and pre-wires the cell reference bookmarks must carry from the start (see 05.1-18-PLAN.md notes, HANDOFF TO PLAN 11)."
files_modified:
...
must_haves:
  truths:
    ...
    - "Persist history (D-06, session-only — clears on app restart AND on a project switch, the disclosed narrowing recorded in Plan 15's gap ledger item (l), R9 review BB21; the failure badge clears on the same switch — D-12's disclosed narrowing, ledger item (m), R10 review CC4) is visible in the Activity accordion, with human-readable outcomes only — never a raw result code (SC1)"
...
switch, a disclosed narrowing per R9 review BB21, Plan 15 ledger item (l); the failure badge clears on the
same switch, D-12's disclosed narrowing, ledger item (m), R10 review CC4) as
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    5. **OWED FROM 05.1-17 — the ack must LAG.** This control is the first UI caller of
       `sendReloadCurrentScene`, so 05.1-17's headline behaviour has never been exercised: its
       completion poll only arms for a HOST_CMD epoch and a local in-game button can never reach it.
       Watch the toolkit side — the result must arrive only once the world is actually rebuilt
       (~1-2s of progressive fill-in), **not** immediately when the call returns. That delay IS the
       feature: it is what makes `ack=1` mean "world rebuilt" and therefore usable as verification
       evidence by Plans 12 and 15. An instant ack means the deferred-ack path regressed.
       Look for `overlay: reload complete — wsIsParsePending 1->0 after N ticks` in DebugView.
    6. **Also owed: coalescing.** Click the reload control 5+ times rapidly. Expect ONE reload, and
       exactly one ack per remote epoch — never a stranded epoch timing out at 11s.
    7. **Bookmarks: plain world x/y/z is sufficient** — v31's `warpPlayer` resolves the destination
       cell internally (verified live in all four directions). Confirm a bookmark saved INSIDE a
       building restores you inside it with the interior rendering solid. Do NOT add a cell reference
       to the bookmark schema; see 05.1-18's HANDOFF TO PLAN 11.
  </how-to-verify>

...
- **V4 (HIGH, Opus, ground-truth confirmed):** the ROUND 4/W1 rollback captured bytes on the WRONG branch (a
  ...
  pre-call bytes) and keyed `applied`/writes by `buildingId` while `decorationPersist.ts:200` resolves the
  mirror path PER STOCK TEMPLATE (`:203`'s own comment) — two buildings sharing a layout share ONE mirror file.
  ...
  <behavior>
    ...
    - **(ROUND 4, W1 — HIGH, Sonnet; DESIGN PICK, vetoable next round)** `reconcileMirrorMode` runs in TWO
      ...
      - **Phase 2 (apply, entered ONLY when phase 1's `failures` is empty):** **(ROUND 5, V4 — HIGH, Opus;
        ground truth: `decorationPersist.ts:200` resolves the mirror path from

...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `reconcileMirrorMode(studioDir: string, overrideDir: string, readVfs: (vfsPath: string) => Buffer,
      ...
      D-09 exists to prevent (the 7/19->7/30 failure night). For each building the scan returns: if its
      `buildingTemplateVfsPath === ''` (unknown — no live capture has ever observed it under the durable-map
      mechanism), do NOT attempt a mirror write/delete for it (there is no stock path to compute the mirror
      filename from) — instead push a fail-closed entry into `failures`:
      `{ buildingId: building.buildingId, error: 'buildingTemplateVfsPath unknown -- no live capture observed
      yet for this building; arm/persist a decoration in it once to populate it' }` and move to the next
      building (do not throw, do not abort the pass). For every OTHER building: if `nextValue === true` and no
      stock-path mirror currently exists on disk, write one (same bytes as the building's current edited
      `.ilf` — reuse the mirror-write logic assembleDecorationEdit already has, factored into the shared
      writeStockMirror/removeStockMirror helpers below); if `nextValue === false`, delete any existing
      stock-path mirror file for that building. Wrap each building's reconcile step (mirror write/delete) in
      its own try/catch — a throw there ALSO becomes a `failures` entry, never aborts the rest of the pass.
    - **(ROUND-3-REVIEW R6, HIGH/MEDIUM, Codex+Opus — the literal D-09 invariant)** After reconciling every
      building, calls `updateWorkspaceMeta(studioDir, { mirrorToStockIlf: nextValue })` ONLY WHEN
      `failures.length === 0`. If ANY building failed to reconcile (either the R1 unknown-path case above or a
      thrown mirror write/delete), the persisted `mirrorToStockIlf` flag is LEFT UNCHANGED (the previous value
      stays in workspace.json) — persisting `nextValue` despite a partial failure would make the flag claim a
      disk state that is FALSE for the failed buildings, reopening the exact D-09 stale-mirror class this
      function exists to close. The function still returns `{ failures }` (non-empty) so the caller (Plan 10)
      can surface which buildings need a retry; the caller is responsible for re-invoking reconcileMirrorMode
      once the underlying issue (e.g. an unknown buildingTemplateVfsPath) is resolved.
    - **(ROUND 3, R8)** `reconcileMirrorMode` returns `{ failures: { buildingId: string; error: string }[] }`
      ...
      persisted, per the R6 rule above); a non-empty array lists exactly which buildings failed and why, AND
      means the flag was NOT persisted this call (R6). This makes the function's own "never throws for one bad
      building, but the flag only moves on a clean pass" contract type-visible to its caller (Plan 10), instead
    ...
    - reconcileMirrorMode never THROWS for one bad building; a partial failure is ALWAYS visible in its return
      value, never only in a log line, and NEVER results in a persisted flag that disagrees with disk (R6).
  ...
  <action>
    ...
    3, R4), and implement reconcileMirrorMode reading `readWorkspaceJson(studioDir).worldEditorBuildingTemplates
    ?? {}` and calling the TWO-arg `scanWorldEditorState(overrideDir, thatMap)` (ROUND-3-REVIEW R1 — never the
    stale one-arg call), pushing a fail-closed `failures` entry for any building whose `buildingTemplateVfsPath
    === ''`, and persisting `updateWorkspaceMeta(studioDir, { mirrorToStockIlf: nextValue })` ONLY when the
    resulting `failures` array is empty (ROUND-3-REVIEW R6), returning `{ failures: { buildingId: string; error:
    string }[] }` (ROUND 3, R8) using scanWorldEditorState + the two new writeStockMirror/removeStockMirror helpers.
  ...
  <acceptance_criteria>
    ...
    argument (spy), the COVERED building's mirror is written/deleted per `nextValue`, and the UNCOVERED
    building produces a `failures` entry naming it (never a thrown VFS-resolution error). ROUND-3-REVIEW R6
    spot-check: the same unknown-path scenario above asserts `updateWorkspaceMeta` is NEVER called with a
    `mirrorToStockIlf` patch (spy shows zero such calls) when `failures.length > 0`; a separate all-clean-pass
    test asserts `updateWorkspaceMeta(studioDir, { mirrorToStockIlf: nextValue })` IS called exactly once when
    `failures` is empty.
  ...
  pass, never lying about a partially-failed reconcile (ROUND-3-REVIEW R6).</done>
</task>

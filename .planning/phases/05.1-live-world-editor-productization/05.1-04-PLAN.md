...
must_haves:
  truths:
    ...
    - "Persist history is session-only, held in memory, and clears on app restart AND on a project switch (D-06 — the project-switch clearing is a disclosed narrowing of D-06's literal 'clears on app restart' wording, an intentional consequence of the cross-project leak fix: switching back to Project A within one app run shows an empty Activity history; recorded in Plan 15's gap ledger item (l), R9 review BB21) (SC2)"
...
**ROUND 10 REVISION (2026-08-01, REVIEWS.md ROUND 8 crew — AA4/AA5):** two forward-plumbing additions Plan 14
needs and no earlier plan supplied: **AA5 (HIGH, Sonnet L2)** — `reset()`, clearing this store's `tree`/
`history`/`sessionOverlay`/`selectedRowId`/`hasFailureBadge` to their initial values in one call, so a project
...
Plan 14's WorldPanel (its own ROUND 10 revision) is the actual caller, on a `studioDir` change. **AA4 (HIGH,
...
- **BB19 (LOW):** the Task 2 read_first's `liveStore.ts:26-31` citation is corrected to `:27-31` (`:26` is
  a comment line; Plans 10/13 already cite `:27-31`).
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - **(ROUND 10, AA5 — HIGH, Sonnet L2; re-derived R9 review, BB2/BB5 — the PROJECT-SWITCH RESET ORDERING
      ...
      involvement. Plan 14's WorldPanel keeps a `[studioDir]` effect ONLY for its component-local refs, its
      ...
      recorded in Plan 15's gap ledger item (l), never silently shipped. **(ROUND 10, AA5 — scoping `refresh()`'s own
  ...
  <action>
    ...
    **(ROUND-3-REVIEW R3)** Also test refresh()'s session-overlay reconciliation directly: seed a tree with
    ...
    imports nothing from this module, so no cycle). Test all three directly: seeding every field non-default
  ...
  <acceptance_criteria>
    ...
    **(R9 review, BB2)** a non-null->different `studioDir` change on `useWorkspaceStore` resets this store
    ...
    between); the initial null->project transition does not.

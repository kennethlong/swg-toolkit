...
must_haves:
  truths:
    ...
    - "worldEditorStore exposes a reset() action clearing tree/history/sessionOverlay/selectedRowId/hasFailureBadge to their initial values, and a lastHostCommandResult field + setLastHostCommandResult(epoch, code) setter — a project-scope module singleton store surviving in the renderer process across the OpenProject flow would otherwise leak one project's Activity log/failure badge into another's session, and Plan 14 needs a way to observe HOST_CMD acks to correlate against sendStartPlacement's returned epoch (Plan 08); Plan 14's WorldPanel is the actual caller of reset() on a project switch, and the actual reader of lastHostCommandResult (REVIEWS.md ROUND 10, AA5/AA4)"
  artifacts:
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - **(ROUND 10, AA5 — HIGH, Sonnet L2, project-switch lifecycle)** `reset(): void` — restores `tree` to `[]`,
      `selectedRowId` to `null`, `sessionOverlay` to a new empty `Map`, `history` to `[]`, and `hasFailureBadge`
      to `false` — the store's full module-initial state. This exists SOLELY for a project switch (no other
      caller this phase ships): `worldEditorStore` is a module-scope singleton that outlives any single
      project's session in the renderer process, so switching projects (via `openWorkspace`,
      `packages/renderer/src/services/workspaceService.ts:219`) without clearing it would leak Project A's
      Activity log/failure badge/session overlay into Project B's session (verified against
      `packages/renderer/src/state/workspaceStore.ts:100-109`'s real `close()` implementation, which touches
      neither this store nor `removeUndoStore`). Plan 14's own ROUND 10 revision (Task 3) is the actual CALLER —
      a `WorldPanel.tsx` effect watching `useWorkspaceStore`'s `studioDir` for a change calls
      `useWorldEditorStore.getState().reset()` (and `useRemoveUndoStore.getState().reset()`, Plan 13's own ROUND
      10 addition) BEFORE the newly-opened project's first `refreshTree()`. This plan's own Task 2 test proves
      ONLY the store-level mechanics; Plan 14 proves the wiring. **(ROUND 10, AA5 — scoping `refresh()`'s own
    ...
    - **(ROUND 10, AA4 — HIGH, Sonnet L1, forward plumbing for Plan 08/14's HOST_CMD ack correlation)**
      ...
      unconditional "active" toast that could be affirmatively wrong (REVIEWS.md AA4).
  ...
  <action>
    ...
    **(ROUND-3-REVIEW R3)** Also test refresh()'s session-overlay reconciliation directly: seed a tree with
    ...
    AA5/AA4)** Add `reset()` (clears `tree`/`history`/`sessionOverlay`/`selectedRowId`/`hasFailureBadge` to
    their module-initial values in one call) and `lastHostCommandResult`/`setLastHostCommandResult(epoch, code)`
    (starts `null`; each call replaces the field with a NEW object, never mutates in place) per the behavior
    spec above, and test both directly: seeding every field non-default then calling `reset()` asserts all five
    are back to initial; calling `setLastHostCommandResult` twice with different values asserts the field holds
    the SECOND call's exact `{ epoch, code }` and is a DIFFERENT object reference than after the first call.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    `null` after refresh(). **(ROUND 10, AA5)** `reset()` restores `tree`/`history` to `[]`,
    `sessionOverlay` to a new empty `Map`, `selectedRowId` to `null`, and `hasFailureBadge` to `false` in one
    call (seed all five non-default, call `reset()`, assert every field is back to its module-initial value).
  ...
  (ROUND-3-REVIEW R3), and the store carries a `reset()` action (AA5, the project-switch mechanism Plan 14's
  WorldPanel calls) and a `lastHostCommandResult` field + setter (AA4, the HOST_CMD ack-correlation groundwork
  Plan 08 writes and Plan 14 reads).</done>
</task>

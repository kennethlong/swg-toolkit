...
- **D-02: Any decoration is removable, stock included.** Remove = the edited `.ilf` omits the row
  ...
  > **Annotated 2026-08-07 (05.1-15 sign-off).** Two narrowings of this wording shipped:
  ...
  > **Live-confirmed** at the checkpoint: three removals, every one data-only, `tatooine.ws`
...
- **D-03: Remove is a World-panel row action guarded by an undo toast.** No confirm dialog — the
  ...
  > **Annotated 2026-08-07 (05.1-15 sign-off).** Two disclosed carve-outs to "guarded by an undo
  ...
  > **Neither was exercised live** — carve-out (1) needs a forced write error to arrange, and the
  > checkpoint's two removals fell >8 s apart so their windows never overlapped and nothing was
  > superseded. Accepted as code-reviewed, unit-tested disclosures. The no-confirm-dialog half and
  > the undo re-add itself WERE live-verified.
...
- **D-06: Persist history is session-only.** The history accordion shows this session's
  ...
  > **Annotated 2026-08-07 (05.1-15 sign-off).** History additionally clears on EVERY project
  ...
  > entry of its own, though it IS a persist by this decision's own wording — live-confirmed at the
  > checkpoint (the Undo'd row's newest entry still reads Remove's message).
...
- **D-09: Flipping the toggle reconciles existing edits.** OFF removes the stock-path shadow
  ...
  > **Annotated 2026-08-07 (05.1-15 sign-off).** "Disk always matches what the toggle claims" holds
  ...
  > hedged in the failure copy rather than fixed this phase — **not exercised live** (reachable only
  > via a forced double fault).
  > **The rest was proven at the byte level**, on a harder case than the plan specifies (2 known +
  > 1 unknown building, with the unknown sharing a mirror path with a known one): the toggle
  > reverted, all five `.ilf` files were byte-identical to their pre-flip controls, both stock
  > mirrors survived, and the flag never moved. Nothing was written even for the buildings that WERE
  > resolvable — the pass is refused whole, never partially applied. The reverse direction was also
  > verified: re-enabling mirror rebuilt the stock file from CURRENT truth, carrying an edit made
  > while mirror was OFF rather than restoring a stale pre-toggle copy.
...
- **D-12: Failure punt = badge + detail waiting.** On a failed persist the World tab gets an
  ...
  > **Annotated 2026-08-07 (05.1-15 sign-off).** Two narrowings: (1) a project switch clears
  ...
  > Neither was exercised live.
  >
  > **⚠ A THIRD narrowing was found live and is the reason SC1 needs restating.** "Raw result codes
  > appear in NEITHER surface" was upheld — but the checkpoint found three ways a failure reached
  > NO surface at all, which is worse than a code and which this decision's wording did not
  > anticipate: `persistDecorationEdit()` returned refusal reasons its only caller discarded;
  > `DecoStripState::Failed` was structurally unreachable because a failed rebind deliberately keeps
  > the arm and `Armed` is tested first; and the orchestrator's abort message went only to the Log
  > tab while this decision's own surface received the bare label `"aborted"`. All three are fixed
  > (commits `1ff6686`, `12c35f2`); two are live-verified. **The intent of SC1 is that a failure is
  > EXPLAINED, not merely that it is worded** — a surface showing nothing, or showing a true-but-
  > empty word, satisfies the letter and fails the decision.

...
type: execute
wave: 3
depends_on: ["05.1-09", "05.1-03", "05.1-05", "05.1-17"]
depends_on_note: "05.1-17 added 2026-08-02 — this plan's ADD checkpoint verifies persistence BY RELOADING."
verification_instrument_changed: |
  ...
  (clientInteriorLayoutManager::refreshInteriorLayout) and is LIVE-VERIFIED in the OCCUPIED Mos
  Eisley cantina: ack code 1, the edit held, the 54 server-owned NPCs were undisturbed, the rest of
  the interior was correct. Evidence: .planning/handoff/2026-08-04-TOOLKIT-CONFIRM-v32-live.md.
  CORRECT INSTRUMENT NOW: persist -> refresh that building -> confirm it took. This works in an
  occupied building. Drive it with HOST_CMD action 7 (REFRESH_INTERIOR):
    packages/harness/scripts/drive-host-command.ps1 -Name '<mapping>' -Action refreshinterior -Id '1082874'
  Prefer this over reload for THIS plan's ADD checkpoint. You no longer need an unoccupied building,
  a zone change, or a relog.
  ⚠ TWO CONSTRAINTS THAT REPLACE THE OLD ONE — both are real and both bite silently:
    1. refreshInteriorLayout and a LIVE PREVIEW DO NOT COMPOSE. Refresh only deletes the building's
       client-only layout objects; our placements carry NetworkIds and survive it. Refresh while a
       preview is up = TWO VISIBLE COPIES. Keep them apart.
    2. NEVER call it while armed (it frees and recreates layout objects -> dangling g_capFocus /
       g_latchedFocus), and gate on wsIsParsePending — mid-parse returns 0, indistinguishable from
       a miss.
  ⚠ AND THE TRAP THAT OUTLIVED THE FIX: an occupied building can produce a FALSE PASS, not just the
  old false failure. The building is kept across a reload WITH its live in-session objects, so
  unsaved work can look persisted. Trust the bytes on disk, not the screen: a placement grows BOTH
  .ilf files by one NODE chunk and leaves .ws UNCHANGED (84 bytes of .ws growth = the duplicate-node
  defect has returned).
  Superseded text (2026-08-02, kept for history): a building with server-owned occupants was KEPT
  across a reload and rendered its PRE-EDIT state until a zone change or relog, so
  "persist, reload, confirm" reported a FALSE FAILURE exactly where the work happens. 05.1-17's
  completion poll did not fix that — it made reload atomic, not correct.
files_modified:
...
must_haves:
  truths:
    ...
    - "REVISED 2026-08-02 (supersedes the preselect-and-validate rule): the placement container is DERIVED FROM THE CLICK POINT, never from the player's location and never from the World-panel picker's preselection. `getContainingBuildingId` is called on the click's own ray hit; whatever building that resolves to IS the container the object is placed in. A click landing in a DIFFERENT building than the picker preselected is placed in the building actually clicked — it is NOT refused as a mismatch — because player position, picker preselection and target position are three independent things and a doorway makes them disagree (maintainer, 05.1-09 checkpoint). Acceptance test, both directions: standing inside the cantina, place just outside the door; standing outside, place just inside the door."
    ...
    - "REVERSED 2026-08-02 — ROUND 3/R10's fail-OPEN-on-0 rule is REMOVED, its premise falsified by live evidence. R10 read building id 0 as the resolver's ambiguous 'could not resolve' sentinel and therefore let the click proceed to spawn. It is not ambiguous: the client catalog states plainly that `world-cell objects report 0 (no PortalProperty)`, so 0 is the AUTHORITATIVE answer 'this click is outside any building' (same finding that retired the stale pre-shim fallback in armDecorationEdit, commit cb6f369). Under the old rule a click just outside a door, during a placement targeting the cantina, would fail open and spawn into the PRESELECTED container — a silent wrong-container write, invisible at placement time and only discovered when the user crosses a portal and the object vanishes. A click resolving to 0 now FAILS CLOSED: refused, nothing spawned, with the words-only strip message naming it as an unbuilt route ('outside a building — exterior placement not wired yet'), NOT as a prohibition. When the `.ws` exterior authoring route lands this branch becomes a route rather than a refusal and nothing else in this plan changes (see .planning/todos/pending/exterior-ws-node-editing.md)."
...
- **R10 (MEDIUM, Fable):** the C6 building-id guard (Task 2) originally left an ambiguous case: what happens
  ...
  > **⚠ SUPERSEDED 2026-08-02 — R10's premise was FALSIFIED by live evidence; do not implement this.**
  > R10 reasoned that `0` is `getContainingBuildingId`'s ambiguous "could not resolve" sentinel. It is
  > not: the client catalog states `world-cell objects report 0 (no PortalProperty)`, so `0` is the
  > AUTHORITATIVE answer "this click is outside any building". The same misreading had produced a stale
  > fallback in `armDecorationEdit` that made every object armable including buildings viewed from
  > outside; it was retired in `cb6f369` after the 05.1-05 checkpoint. Implementing R10 as written would
  > let a click outside a doorway spawn into the PRESELECTED container — a wrong-container write that is
  > invisible at placement time and only surfaces when the user crosses a portal and the object
  > disappears (observed live, 05.1-09 checkpoint). Task 2 now FAILS CLOSED on a resolved `0` and derives
  > the container from the click. Kept here as review history, not as instruction.
...
<task type="auto">
  ...
  <behavior>
    ...
    - Placement mode is exclusive with an already-armed decoration edit (if `g_capArmed` is true, a
      ...
      would strand the renderer's pending-ack slot until its ~11 s timeout fires (PLACEMENT_ACK_TIMEOUT_MS =
      11_000, Plan 08's R10/CC3 latency-aware derivation), turning an instant
...
<task type="auto">
  ...
  <behavior>
    - While `g_placementActive` and the left mouse button is clicked over the WORLD (not an ImGui surface,
      ...
      hit is valid:
      **(CONTAINER RESOLUTION — REVISED 2026-08-02, supersedes the C6 preselect-and-validate guard and
      ROUND 3/R10's fail-open rule; maintainer decision at the 05.1-09 checkpoint.)**

      Resolve the container FROM THE CLICK, and place the object in whatever that resolves to. Call
      `swg::endpoints::getContainingBuildingId` on the click's own ray hit (the SAME resolver
      `armDecorationEdit` already calls — pass the ray-hit object through the identical resolution path)
      and bind the result to a local `clickBuildingId`. **`clickBuildingId` IS the container.** Do NOT
      compare it to `g_placementBuildingId`; do NOT derive the container from the player's position.

      Three branches, and only three:

      - `clickBuildingId != 0` → **place it there.** Even if it differs from the World panel picker's
        preselection. Player position, picker preselection and target position are three INDEPENDENT
        inputs, and a doorway makes them disagree — standing in the cantina doorway a user can
        legitimately click just outside, and standing outside can legitimately click just inside. The
        old "wrong building — click inside the building you selected" refusal is REMOVED: it made a
        legitimate action an error.
      - `clickBuildingId == 0` → **FAIL CLOSED. Refuse, spawn nothing.** `0` is NOT an ambiguous
        "resolver could not answer" sentinel — the client catalog states that `world-cell objects report
        0 (no PortalProperty)`, so it is the AUTHORITATIVE answer "this click is outside any building"
        (the same finding that retired the stale pre-shim fallback in `armDecorationEdit`, `cb6f369`).
        Show a one-shot words-only strip message naming it as an UNBUILT ROUTE, not a prohibition —
        "outside a building — exterior placement not wired yet" — and leave placement mode ACTIVE.
        Rationale for fail-closed: under R10's fail-OPEN rule this click spawned into the PRESELECTED
        container, a silent wrong-container write that looks correct at placement time and is only
        discovered when the user crosses a portal and the object vanishes. That exact symptom was
        observed live (maintainer, 05.1-09 checkpoint). A refusal the user sees immediately is strictly
        better than a wrong write they find three rooms later.
      - resolver UNBOUND (`getContainingBuildingId == nullptr`) → refuse with the existing
        "endpoint unresolved" class of message. This is genuinely "cannot answer" and is distinct from
        a resolved `0`; do NOT collapse the two cases as R10 did.

      When the `.ws` exterior authoring route lands, the `== 0` branch becomes a ROUTE rather than a
      refusal and nothing else in this task changes. See
      `.planning/todos/pending/exterior-ws-node-editing.md` for the maintainer's design intent (the
      interior/exterior split must be invisible to the user; the editor picks the route).

      The residual same-building-WRONG-CELL risk is still NOT addressed here — no advertised endpoint can
      resolve a cell name yet (Plan 15's change-request).
      **(ROUND 3, R3 ripple — REVISED 2026-08-02: resolve for `clickBuildingId`, NOT
      `g_placementBuildingId`.)** Before calling `wsAddObject`, ALSO resolve `g_capBuildingTemplate` for
      **`clickBuildingId`** (the building actually clicked, per the container resolution above — resolving
      it for the preselected id would write an .ilf row describing the WRONG building's template, which is
      the same wrong-container class of defect the fail-closed rule above exists to prevent)
      using the SAME `getObjectByIdAdvertised(&bldgId)` + `getTemplateFilename(bldg)`
      ...
      Then: call `wsAddObject(g_placementTemplate, currentPreviewTransform12, clickBuildingId)`
      — **`clickBuildingId`, not `g_placementBuildingId`** (2026-08-02 revision). Passing the preselected
      id here is exactly the bug: `containedById` is what decides cell membership, and a wrong value
      produces an object that renders correctly from where it was placed and then vanishes across a
      portal boundary. `overlay.cpp:1247`'s "Insert at player" hardcodes `0` (= world) and is the live
      demonstration of that symptom — do NOT copy its container argument, only its call shape
  ...
  <done>(REVISED 2026-08-02) The placement container is derived from the CLICK, not the preselection or the
  player: a click inside any building spawns into THAT building (cross-building clicks are allowed, not
  refused), and a click outside any building — resolved id `0`, the authoritative "not in a POB" answer —
  fails CLOSED with a words-only "exterior placement not wired yet" message rather than silently spawning
  into the preselected container. A valid click spawns live, auto-arms exactly like an existing decoration
  edit, persists as an appended row, and the temp preview node is despawned after a successful rebind+save,
  matching RESEARCH's own verified design. The placed object survives a portal crossing — the check that
  proves the container was right, since a wrong container looks correct until the user walks through a
  doorway.</done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    4. **(REVISED 2026-08-02 — the old "wrong building refusal" step is REPLACED; that behavior was
       removed as a defect, so testing for it would now pin the wrong contract.)** Two sub-checks, and
       they are the acceptance test for the whole container-resolution change:
       a. **Cross-building placement is ALLOWED.** Click the floor in a DIFFERENT building than the one
          whose id was passed. Confirm the object DOES spawn, in the building you clicked. No refusal.
       b. **The doorway, both directions** — this is the case that separates target from player:
          standing INSIDE the cantina, click just OUTSIDE the door → must be REFUSED with
          "outside a building — exterior placement not wired yet", nothing spawned, placement still
          active. Then standing just OUTSIDE the door, click just INSIDE → must SPAWN, contained by the
          cantina's cell.
       The critical follow-up for (b)'s second case: **walk across the portal boundary and confirm the
       object stays visible from both sides.** An object placed into the wrong container renders
       correctly from where it was placed and vanishes when you cross a portal — that is the live
       symptom this revision exists to prevent, and it is invisible at placement time.
...
| T-05.1-12e | Tampering | placement into the WRONG container (object written to a cell it is not physically in) | mitigate (partial — building-level only) | **REVISED 2026-08-02.** The container is DERIVED from the click's own `getContainingBuildingId` result rather than validated against a preselection, so a cross-building click is placed correctly rather than refused, and a click outside any building (resolved id `0`) FAILS CLOSED instead of silently spawning into the preselected container. This removes the wrong-container WRITE that the previous fail-open rule permitted. Residual, unchanged: same-building-wrong-ROOM (a different cell within the SAME building) is NOT closable with currently-advertised endpoints — no cell-name resolver exists in the client's 150-row catalog (verified 2026-08-02). Disclosed here and in Plan 14/15; permanent fix is Plan 15's change-request. **Why this matters more than a normal mis-write:** a wrong container is invisible at placement time — the object renders correctly from where it was placed and only disappears once the user crosses a portal boundary, so the error is discovered long after it is made. |
</threat_model>

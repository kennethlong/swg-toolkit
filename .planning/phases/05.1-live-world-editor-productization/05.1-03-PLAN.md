...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    LiveState); `grep -n "ls->captureKind\s*=\s*cap->kind" packages/live-inject/agent/channel.cpp` matches,
...
<task type="auto">
  ...
  <read_first>
    ...
    `static_assert(sizeof(LiveState) == 1308, ...)` block and its 17 `offsetof` asserts through `resultEpoch`

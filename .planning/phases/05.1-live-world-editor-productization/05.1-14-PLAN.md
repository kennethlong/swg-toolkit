...
cellName from; NOTE: this tree ONLY contains buildings with an existing edit_*.ilf on disk, per D-05 — never-
edited/stock-only buildings are absent, per this revision's C6/Fable disclosure):
```typescript
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Clicking "+ Add decoration…" resolves a target building/cellName: if a building or decoration row is
      ...
      existing decorations. **(C6, Fable facet — explicit scope disclosure):** the selectable set here is
    ...
    - Two-surface confirm (021-A Frame 3): once a subsequent CAPTURE/RESULT cycle lands with
      ...
      **Plan 08 Task 3 built** (added in the 2026-07-31 revision specifically to close this gap; before that
      ...
      is to (a) ensure a `refresh()` is triggered promptly after that history growth while the World tab is
      open (e.g., a small effect subscribing to the store's history length and re-running refresh on growth)
      so the new row appears without requiring a manual reopen of the tab, AND (b) **(Sonnet F9/Cursor M5)**

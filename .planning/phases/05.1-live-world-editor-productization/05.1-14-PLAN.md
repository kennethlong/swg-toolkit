...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Two-surface confirm (021-A Frame 3): once a subsequent CAPTURE/RESULT cycle lands with
      `capture.kind === 'add'` and a successful result (observable via the existing poll loop's
      handleDecorationCapture → worldEditorStore.recordPersistResult path, which this plan does not need to
      re-wire — it already fires per Plan 06/Plan 04's existing integration), the World panel's next
      `refresh()` shows the new row. This plan's job is only to ensure a `refresh()` is triggered promptly
      after a HOST_CMD or decoration RESULT lands while the World tab is open (e.g., a small effect
      subscribing to the store's history length and re-running refresh on growth) so the new row appears
      without requiring a manual reopen of the tab.
  </behavior>
  <action>
    ...
    "Placement mode active" toast; assert a history-length change triggers a fresh refresh() call (spy on
    worldEditorScan or the store's refresh action).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    or empty one; the World panel refreshes automatically once a new persist result lands, without requiring
    a manual tab close/reopen.
  </acceptance_criteria>

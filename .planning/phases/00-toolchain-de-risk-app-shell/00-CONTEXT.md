...
- **D-02:** The native `.node` addon runs in a **dedicated Electron utility process** (spawned from main), not the main process and never the sandboxed renderer. Rationale: isolates native crashes (memory injection, parsing untrusted binaries) from the window. Accept the added `MessagePort` hop for control-channel calls. The SharedArrayBuffer data channel is transferred from this utility process to the renderer.
...
- **D-04:** Phase 0's wiring proof goes **all the way to a real zero-copy SharedArrayBuffer round-trip** — C++ (utility process) allocates/fills a `SharedArrayBuffer`, transfers it to the renderer via `MessageChannel`, and the renderer reads it on an animation frame with `crossOriginIsolated === true`. De-risks the hardest mechanism (the 60fps data channel) now rather than at the live-sync phase. A trivial "hello" call alone is **not** sufficient for this phase.
...
- Security defaults: `contextIsolation: true`, `nodeIntegration: false`, narrow typed validated preload bridge.

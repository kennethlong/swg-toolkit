...
| 011 | viewport-gizmo | How do the viewport HUD + transform gizmo + live-sync state read — the Phase-5 WYSIWYG "drag a gizmo → moves in client" surface? | **B · Full overlay HUD (live-sync card + gizmo-mode bar + transform readout)** · **+ B2 revert/guard (approved 2026-07-07, folds into B)** | viewport, gizmo, live-sync, hud, wysiwyg, phase-5, phase-3 |
| 014 | datatable-grid-editor | How does the full DTII spreadsheet editor read — typed columns, row ops, cell editing, modified state, staging, and the byte-exact round-trip gate (pass / running / FAIL-with-byte-diff)? | **D · Synthesis (B schema rail + C Grid\|Hex toggle)** | datatable, dtii, grid, editor, typed-editors, round-trip-gate, phase-5, phase-8 |
...
- ~~**014 datatable / DTII grid editor**~~ — **BUILT (in review, 2026-07-07)** — see table row 014.
